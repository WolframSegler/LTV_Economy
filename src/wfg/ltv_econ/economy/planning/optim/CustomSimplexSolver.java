 ...
 */
package wfg.ltv_econ.economy.planning.optim;
...
import org.apache.commons.math4.legacy.optim.linear.LinearOptimizer;
 ...
 * @since 2.0
 */
public class CustomSimplexSolver extends LinearOptimizer {
    /** Default amount of error to accept in floating point comparisons (as ulps). */
    public static final int DEFAULT_ULPS = 10;

    /** Default cut-off value. */
    public static final double DEFAULT_CUT_OFF = 1e-10;
    ...
    protected PivotSelectionRule pivotSelection;
     ...
     */
    protected SolutionCallback solutionCallback;

    protected SimplexTableau previousTableau;

    public CustomSimplexSolver() {
        this(DEFAULT_EPSILON, DEFAULT_ULPS, DEFAULT_CUT_OFF);
    }

    public CustomSimplexSolver(final double epsilon) {
        this(epsilon, DEFAULT_ULPS, DEFAULT_CUT_OFF);
    }

    public CustomSimplexSolver(final double epsilon, final int maxUlps) {
        this(epsilon, maxUlps, DEFAULT_CUT_OFF);
    }

    public CustomSimplexSolver(final double epsilon, final int maxUlps, final double cutOff) {
        this(epsilon, maxUlps, cutOff, null);
    }

    public CustomSimplexSolver(final double epsilon, final int maxUlps, final double cutOff,
        final SimplexTableau previousTableau
    ) {
        this.epsilon = epsilon;
        this.maxUlps = maxUlps;
        this.cutOff = cutOff;
        this.pivotSelection = PivotSelectionRule.DANTZIG;
        this.previousTableau = previousTableau;
    }
     ...
     */
    protected void solvePhase1(final SimplexTableau tableau) throws
    ...
    /** {@inheritDoc} */
    @Override
    public PointValuePair doOptimize() throws
        TooManyIterationsException,
        UnboundedSolutionException,
        NoFeasibleSolutionException
    {
        ...
        final SimplexTableau tableau = new SimplexTableau(
            getFunction(), getConstraints(), getGoalType(),
            isRestrictedToNonNegative(),
            epsilon, maxUlps
        );
        if (previousTableau != null) {
            tableau.copySolutionFrom(previousTableau);
        }
    ...
    public final SimplexTableau getPrevTableau() {

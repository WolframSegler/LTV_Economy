...
public class LtvLocalResourcesSubmarketPlugin extends LtvBaseSubmarketPlugin
		implements EconomyUpdateListener, EconomyTickListener {
	...
	public LtvLocalResourcesSubmarketPlugin() {
	...
	public boolean shouldHaveCommodity(CommodityOnMarketAPI commodity) {
		if (market.isIllegal(commodity)
				&& commodity.getCommodityMarketData().getMarketShareData(market).isSourceIsIllegal()) {
			return false;
		}
		return true;
	}
	...
	public boolean isIllegalOnSubmarket(CargoStackAPI stack, TransferAction action) {
		if (stack.getCommodityId() == null)
			return true;
		if (stack.getResourceIfResource().hasTag(Commodities.TAG_NON_ECONOMIC))
			return true;
		return false;
	}
	...
	public void reportPlayerMarketTransaction(PlayerMarketTransaction transaction) {
		// addAndRemoveStockpiledResources(0f, true, false, false); // not needed b/c
		// economyUpdated() gets called
	...
	public int getEstimatedShortageCounteringCostPerMonth() {
		...
		for (CommodityOnMarketAPI com : all) {
			...
			float units = LtvLocalResourcesSubmarketPlugin.getDeficitMonthlyCommodityUnits(com);
			units = Math.min(units, cargo.getCommodityQuantity(com.getId()));
			units -= taken.getCommodityQuantity(com.getId());
			if (units > 0) {
				float per = LtvLocalResourcesSubmarketPlugin.getStockpilingUnitPrice(com.getCommodity(), true);
				totalCost += units * per;
			}
		}
	...
	public static int getStockpilingUnitPrice(CommoditySpecAPI spec, boolean forShortageCountering) {
		...
		return Math.max(Math.round((spec.getBasePrice() * mult)), 0);
	}

	public static float getDeficitMonthlyCommodityUnits(CommodityOnMarketAPI com) {

		StatMod mod = com.getAvailableStat().getFlatMods().get(Submarkets.LOCAL_RESOURCES);
		float modAlready = (mod != null) ? mod.value : 0;

		int demand = com.getMaxDemand();
		int available = Math.round(com.getAvailable() - modAlready);
	...
	public void reportEconomyTick(int iterIndex) {
		...
		if (market.isPlayerOwned()) {
			...
			for (CargoStackAPI stack : taken.getStacksCopy()) {
				...
				addToTooltipCargo = Math.max(addToTooltipCargo, 1.0f - tooltipCargo.getCommodityQuantity(cid));
	...
	public String getTariffValueOverride() {
		if (preTransactionCargoCopy == null)
			return null; // could happen when visiting colony from colony list screen
		CargoAPI cargo = getCargo();
	...
	protected void createTooltipAfterDescription(TooltipMakerAPI tooltip, boolean expanded) {
		...
		tooltip.addSectionHeading("Stockpiled per month", market.getFaction().getBaseUIColor(),
